...
st.write("Predict the face that is being represented in the image.")

model = load_model("model.h5")
l=['Jennie', 'Jisoo', 'Rose', 'Lisa', 'Others']

model = load_model("model.h5")

uploaded_file = st.file_uploader(
    "Upload an image of a blackpink :", type="jpg"
)
predictions=-1
...
if sample_img_choice:
    image1 = Image.open("jfrexwcedu.jpg")
    image1=image.smart_resize(image1,(256,256))
    img_array = image.img_to_array(image1)
    img_array = np.expand_dims(img_array, axis=0)
    img_array = img_array/255.0
